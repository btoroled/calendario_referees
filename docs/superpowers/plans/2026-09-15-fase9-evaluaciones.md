...
**Limitación conocida, heredada a Fase 10:** por la regla de "pendiente" de este plan (Global Constraints), un partido sale de `/evaluaciones` en cuanto tiene **cualquier** evaluación cargada — así que en la práctica solo el primer tipo enviado (de los 4: performance/fisico/videoanalisis/coaching) llega a cargarse para un partido dado; los otros tres nunca tienen una pantalla que los ofrezca, porque `crearEvaluacion` no tiene otro punto de entrada en el repo. Es una simplificación deliberada de este plan, no un bug, pero Fase 10 (perfil de referee consolidado `/referees/[refereeId]`) debería decidir explícitamente si abre un segundo punto de carga para los tipos restantes de un partido ya evaluado, o si se acepta el límite de "un tipo por partido en la práctica" — no asumir que está resuelto. `actions/evaluaciones.ts::listEvaluacionesDeReferee` ya existe (construido en Task 2, revisado y aprobado, sin consumidor hoy) y es la lectura natural para ese perfil.
