-- Nota de diseño: `perfil` NO tiene policies de insert/update. Solo se puede
-- crear o modificar una fila de perfil con el service-role key (bypass de RLS),
-- porque el MVP no tiene alta de usuarios autoservicio: un admin/comité
-- aprovisiona cuentas fuera de la app (ver scripts/rls-test para el patrón).
-- Si una fase futura agrega una pantalla de "invitar usuario" para el comité,
-- esa acción deberá seguir usando el service-role del lado del servidor con
-- una verificación de rol explícita en código (mismo patrón que el resto de
-- las server actions), no una policy de insert/update abierta a perfil.
...
using (
  fn_rol() = 'admin_nacional' and (
    pais_id = fn_pais_id()
    or region_id in (select id from region where pais_id = fn_pais_id())
  )
